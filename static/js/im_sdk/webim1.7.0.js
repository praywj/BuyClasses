/*eslint-disable*/
// var _convert_txmsg = require('~cscommon/utils/convert');
...
(function(webim) {
    ...
    //最大允许长轮询返回错误次数
    var LONG_POLLING_MAX_RET_ERROR_COUNT = 20;
    ...
    var MsgManager = new function() {
            ...
            this.getC2CHistoryMsgs = function(options, cbOk, cbErr) {
                ...
                if (options.MaxCnt > 50) {
                    if (cbErr) {
                        cbErr(tool.getReturnError("MaxCnt can not be greater than 50", -15));
                        return;
                    }
                    return;
                }
            ...
            this.init = function(listeners, cbOk, cbErr) {
                ...
                onMsgCallback = function () {
                    try {
                        var txMsgs = arguments[0];
                        var ecMsgs = txMsgs.map(_convert_txmsg.msgData);

                        window.ecim && window.ecim.reportTxMsg(ecMsgs);
                    } catch(e) {}

                    listeners.onMsgNotify.apply(null, arguments);
                };
